...
TOPOS = {
    'karate_club': {'graph': nx.karate_club_graph(), 'directed': False},
    'davis_southern': {'graph': nx.davis_southern_women_graph(), 'directed': False},
    'florentine_families': {'graph': nx.florentine_families_graph(), 'directed': False},
}
...
def get_seed_set(graph, budget, opinion, ffm):
    ...
    greedy_seeds  = Algorithms.greedy_OM(graph, budget, opinion, ffm, graph.number_of_nodes(), 10)
    ...
    seed_set = [(degree_seeds, 'Degree'), (degdis_seeds, 'DegreeDiscount'), (irie_seeds, 'IRIE'), (om_irie_seeds, 'OM-IRIE'), (op_deg_seeds, 'OpinionDegree'), (random_seeds, 'Random')]
    return seed_set
...
if __name__ == '__main__':
    ...
    start = time.time()
    range_experiment(range(0, k_max+1, k_delta), n_iters, n_trials, t_horizon, 'SMALL-REAL')
    end = time.time()
